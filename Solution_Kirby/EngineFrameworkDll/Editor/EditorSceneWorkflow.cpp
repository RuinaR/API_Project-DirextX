...
bool EditorSceneWorkflow::SaveCurrentSceneDataWithBaseline(const std::string& sceneName)
{
	...
	return CaptureSceneSnapshot(sceneName);
}
...
bool EditorSceneWorkflow::SaveSceneDataAs(const std::string& sceneName)
{
	...
	if (!SaveCurrentSceneDataWithBaseline(sceneName))
	{
		return false;
	}
