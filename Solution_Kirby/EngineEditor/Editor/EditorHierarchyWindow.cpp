...
namespace
{
	...
	bool IsCurrentSceneDirty()
	{
		...
		return !sceneName.empty() && EditorSceneWorkflow::IsSceneDirty(sceneName);
	}
	...
	bool ExecutePendingSceneAction()
	{
		...
		{
			succeeded = EditorSceneWorkflow::CreateNewScene();
		...
		{
			succeeded = EditorSceneWorkflow::OpenSceneData(g_pendingOpenSceneName);
...
void EditorHierarchyWindow::Draw()
{
	...
	if (WindowFrame::GetInstance() != nullptr && WindowFrame::GetInstance()->GetRenderType() == RenderType::Edit)
	{
		...
		const bool isDirty = !sceneNameString.empty() && EditorSceneWorkflow::IsSceneDirty(sceneNameString);
		...
		if (ImGui::Button("Save Scene"))
		{
			if (sceneName != nullptr && strlen(sceneName) > 0)
			{
				const bool saved = EditorSceneWorkflow::SaveCurrentSceneDataWithBaseline(sceneNameString);
...
void EditorHierarchyWindow::DrawSaveSceneAsPopup()
{
	...
	if (ImGui::Button("Save"))
	{
		...
		{
			const bool saved = EditorSceneWorkflow::SaveSceneDataAs(newSceneName);
...
void EditorHierarchyWindow::DrawSceneDirtyConfirmPopup()
{
	...
	{
		const std::string sceneName = GetCurrentSceneName();
		const bool saved = !sceneName.empty() && EditorSceneWorkflow::SaveCurrentSceneDataWithBaseline(sceneName);
		std::cout << (saved ? "Save Scene succeeded: " : "Save Scene failed: ") << sceneName << std::endl;
		if (saved)
		{
			QueuePendingSceneAction();
